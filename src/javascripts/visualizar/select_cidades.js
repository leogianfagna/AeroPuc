...
function imprimirDadosNaTabela(){
    ...
    fetch(`http://localhost:3000/queryTabelasAdministrativas?tabelaParaExecutarSelect=${"cidades"}`)
        .then(response => response.json())
        .then(data => {
            if (data.status === 'SUCCESS') {
                let i = 0;
                const tabelaDeCidades = document.getElementById('tabelaDeCidades');
                tabelaDeCidades.innerHTML = ''; // Limpa o conteúdo atual
                ...
                data.payload.forEach(rowData => {
                    ...
                    tr.id = i;
                    i++;
                    tabelaDeCidades.appendChild(tr);
                });
