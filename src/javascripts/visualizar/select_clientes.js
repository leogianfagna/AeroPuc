    ...
    fetch(`http://localhost:3000/queryTabelasAdministrativas?tabelaParaExecutarSelect=${"cliente"}`)
        ...
        .then(data => {
            if (data.status === 'SUCCESS') {
                let i = 0;
                const tabelaDeClientes = document.getElementById('tabelaDeClientes');
                tabelaDeClientes.innerHTML = ''; // Limpa o conteúdo atual
                ...
                data.payload.forEach(rowData => {
                    ...
                    tr.id = i;
                    i++;
                    tabelaDeClientes.appendChild(tr);
                });
